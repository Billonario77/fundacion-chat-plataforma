    ...
    `;
        ...
        if (estado === 'completado' && req.user?.rol === 'usuario') {
            ...
            if (guiaIdFromTurno) {
                ...
                (0, socketService_1.notificarUsuario)(guiaIdFromTurno, 'estado-turno-actualizado', {
                    turnoId: turnoId,
                    estado: estado,
...
const completarMisDatos = async (req, res) => {
    try {
        ...
        const { primerNombre, segundoNombre, primerApellido, segundoApellido, cedula, edad, rh, sexo, telefonoFijo, celular, estatura, peso, direccion, ciudad, tipoAdiccion, observaciones, contactoEmergencia } = req.body;
      ...
      RETURNING *
    `;
        const result = await connection_1.pool.query(query, [...valores, userId]);
        if (result.rows.length === 0) {
            res.status(404).json({
                success: false,
...
const reprogramarTurno = async (req, res) => {
    try {
        ...
        const turnoQuery = await connection_1.pool.query(`SELECT t.*, u.id as usuario_id_verify
       FROM turnos t
       JOIN usuarios u ON t.usuario_id = u.id
