...
exports.actualizarMiFoto = exports.completarMisDatos = exports.actualizarFotoPerfil = exports.getMiPerfil = exports.getMiGuiaActual = exports.getHistorialAdmin = exports.obtenerMetricasCancelaciones = exports.obtenerCancelacionesAdmin = exports.contarCancelacionesNoVistas = exports.hayCancelacionesNoVistas = exports.marcarCancelacionesComoVistas = exports.getMisReprogramaciones = exports.reprogramarTurno = exports.cancelarTurno = exports.getHistorialTurnos = exports.misSolicitudes = exports.obtenerTurnoPorId = exports.actualizarEstadoTurno = exports.misTurnos = exports.solicitarApoyo = void 0;
...
const solicitarApoyo = async (req, res) => {
        ...
        if (esPrimeraVez) {
         ...
         LIMIT 1`, [usuarioId]);
           ...
           LIMIT 1`, [usuarioId]);
            ...
            if (!guiaAsignado) {
                const guiasDisponibles = await connection_1.pool.query('SELECT id FROM usuarios WHERE rol = $1 AND disponible = true ORDER BY random() LIMIT 1', ['guia']);
    ...
    `;
        ...
        if (esPrimeraVez) {
            ...
            const guiaNombreQuery = await connection_1.pool.query('SELECT nombre FROM usuarios WHERE id = $1 AND rol = $2', [guiaAsignado, 'guia']);
...
const actualizarEstadoTurno = async (req, res) => {
    try {
      ...
      SET estado = $1,
          ...
          cancelado_por = $3
    `;
        const params = [estado, estado === 'cancelado' ? motivo : null, estado === 'cancelado' ? req.user?.rol : null];
        if (estado === 'iniciado') {
...
const getMiGuiaActual = async (req, res) => {
    try {
        const usuarioId = req.user?.id;
        if (!usuarioId) {
            res.status(401).json({ error: 'No autenticado' });
...
const actualizarFotoPerfil = async (req, res) => {
    ...
    `;
        const result = await connection_1.pool.query(query, [foto_perfil, usuarioId]);
        if (result.rows.length === 0) {
            res.status(404).json({ error: 'Usuario no encontrado' });
            return;
        }
...
const completarMisDatos = async (req, res) => {
    try {
        ...
        const verificar = await connection_1.pool.query('SELECT datos_completados FROM usuarios WHERE id = $1', [usuarioId]);
        ...
        const { primer_nombre, segundo_nombre, primer_apellido, segundo_apellido, telefono, celular, cedula, edad, rh, sexo, estatura, peso, direccion, ciudad, tipo_adiccion, observaciones, cto_emerg_nombre, cto_emerg_celular, cto_emerg_email } = req.body;
...
exports.completarMisDatos = completarMisDatos;
