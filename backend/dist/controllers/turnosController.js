...
`nimport { AuthRequest } from '../middleware/auth';
...
AND;
estado;
IN('pendiente', 'aceptado', 'iniciado');
AND((fecha_programada < $2), AND(fecha_programada + (COALESCE(duracion_minutos, 60) * interval), '1 minute')) > $3;
OR(fecha_programada >= $3, AND, fecha_programada < $2);
`,
...
en;
ese;
horario($, { fechaInicio, : .toLocaleString() } - $, { fechaFin, : .toLocaleString() }).Por;
favor, elige;
otra;
fecha;
u;
...
INSERT;
INTO;
turnos(usuario_id, guia_id, fecha_programada, estado, modalidad, requiere_asignacion_admin, created_at);
...
$;
{
    rol === 'crisis' ? '🆘 crisis' : rol === 'apoyo' ? '🌱 apoyo' : '📋 seguimiento';
}
`
      });
    }
...
$;
{
    estado === 'cancelado' ? 'cancelado' : 'actualizado';
}
el;
turno `
      });
...
INSERT;
INTO;
auditoria_logs(usuario_afectado_id, guia_afectado_id, accion, detalles, created_at);
VALUES($1, $2, $3, $4, NOW()) `,
      [
        usuarioId,
        req.user?.id,
...
id = $3;
RETURNING;
id, estado, fecha_programada, cancelado_por `;
...
t.id = $1 `;
    ...
    if (preferencia === 'mismo_guia') {
      ...
      const verificarDisponibilidad = await pool.query(
        `;
SELECT;
id, fecha_programada, duracion_minutos;
FROM;
turnos;
WHERE;
guia_id = $1;
...
$;
{
    fechaConflicto.toLocaleTimeString();
}
a;
$;
{
    finConflicto.toLocaleTimeString();
}
Por;
favor, elige;
otro;
...
INSERT;
INTO;
turnos(usuario_id, guia_id, fecha_programada, duracion_minutos, modalidad, estado, es_reprogramacion, turno_original_id, created_at);
...
INSERT;
INTO;
reprogramaciones(turno_original_id, usuario_id, preferencia, fecha_preferida, comentarios, estado, nuevo_turno_id, created_at);
...
INSERT;
INTO;
auditoria_logs(usuario_afectado_id, guia_afectado_id, accion, detalles);
VALUES($1, $2, $3, $4) `,
        [
          usuarioId,
          guiaOriginalId,
...
INSERT;
INTO;
reprogramaciones(turno_original_id, usuario_id, preferencia, fecha_preferida, comentarios, estado);
...
INSERT;
INTO;
auditoria_logs(usuario_afectado_id, accion, detalles);
VALUES($1, $2, $3) `,
      [
        usuarioId,
  ...
  try {
    ...
    if (rol === 'usuario') {
      ...
      let result = await pool.query(
        `;
UPDATE;
ultima_visto_cancelaciones;
SET;
ultima_visualizacion = NOW(), updated_at = NOW();
WHERE;
usuario_id = $1 `,
        [usuarioId]
      );
...
INSERT;
INTO;
ultima_visto_cancelaciones(usuario_id, ultima_visualizacion);
VALUES($1, NOW()) `,
          [usuarioId]
        );
      }
...
INSERT;
INTO;
ultima_visto_cancelaciones(guia_id, ultima_visualizacion);
VALUES($1, NOW()) `,
          [usuarioId]
        );
      }
...
uv;
ON;
uv.usuario_id = t.usuario_id;
WHERE;
t.usuario_id = $1;
AND;
t.estado = 'cancelado';
AND;
t.cancelado_por = 'usuario';
AND(uv.ultima_visualizacion, IS, NULL, OR, t.created_at >= uv.ultima_visualizacion) `, [usuarioId]);
...
uv;
ON;
uv.guia_id = t.guia_id;
WHERE;
t.guia_id = $1;
AND;
t.estado = 'cancelado';
AND;
t.cancelado_por = 'guia';
AND(uv.ultima_visualizacion, IS, NULL, OR, t.created_at >= uv.ultima_visualizacion) `, [usuarioId]);
      cancelacionesNoVistas = parseInt(result.rows[0].count);
    } else {
      res.status(403).json({ error: 'Rol no autorizado' });
...
export const getMiPerfil = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const query = `;
SELECT;
id, nombre, email, foto_perfil, rol, datos_completados;
FROM;
usuarios;
WHERE;
id = $1 `;
