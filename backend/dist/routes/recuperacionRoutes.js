...
router.post('/solicitar', async (req, res) => {
    try {
        ...
        const usuarioQuery = await connection_1.pool.query('SELECT id, email, nombre FROM usuarios WHERE email = $1', [email]);
        ...
        await connection_1.pool.query(`INSERT INTO recuperacion_codigos (email, codigo, expira)
       VALUES ($1, $2, $3)`, [email, codigo, expira]);
        try {
...
router.post('/verificar', async (req, res) => {
    ...
    `;
        ...
        const result = await connection_1.pool.query(query, [email, codigo]);
        ...
        await connection_1.pool.query('UPDATE recuperacion_codigos SET usado = true WHERE id = $1', [result.rows[0].id]);
        await connection_1.pool.query('UPDATE usuarios SET password_hash = crypt($1, gen_salt($2)) WHERE email = $3', [nuevaPassword, 'bf', email]);
...
//# sourceMappingURL=recuperacionRoutes.js.map