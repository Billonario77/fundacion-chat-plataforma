...
const apoyoQueue_1 = __importDefault(require("../queues/apoyoQueue"));
const connection_1 = require("../database/connection");
const socketService_1 = require("../services/socketService");
console.log('🚀 Worker de apoyo iniciado...');
apoyoQueue_1.default.process(async (job) => {
    ...
    try {
        ...
        if (fechaPreferida) {
            ...
            const turnosUsuario = await connection_1.pool.query(`SELECT id, guia_id, estado FROM turnos 
        WHERE usuario_id = $1 
        AND estado IN ('pendiente', 'aceptado', 'iniciado')
        AND (
    ...
    `;
        ...
        if (esPrimeraVez) {
            ...
            (0, socketService_1.notificarAAdmins)('nuevo-turno-para-asignar', {
                turnoId: turnoId,
                usuarioId: usuarioId,
                tipo: tipo,
...
apoyoQueue_1.default.on('completed', (job, result) => {
    console.log(`🎉 Trabajo ${job.id} completado. Resultado:`, result);
});
apoyoQueue_1.default.on('failed', (job, err) => {
    console.error(`❌ Trabajo ${job.id} falló:`, err);
});
//# sourceMappingURL=apoyoWorker.js.map