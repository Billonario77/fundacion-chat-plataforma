...
export interface Cobro {
  ...
  metodo_pago?: 'mercadopago' | 'epay' | 'transferencia' | 'efectivo' | 'bolsa_horas';
...
export class CobroModel {
  ...
  async crear(datos: Omit<Cobro, 'id' | 'created_at' | 'updated_at'>): Promise<Cobro> {
    const query = `
      INSERT INTO cobros (
  ...
  async obtenerPorId(id: string): Promise<Cobro | null> {
    const query = 'SELECT * FROM cobros WHERE id = $1';
    const result = await this.pool.query(query, [id]);
    return result.rows[0] || null;
  }

  async obtenerPorTurnoId(turnoId: string): Promise<Cobro | null> {
    const query = 'SELECT * FROM cobros WHERE turno_id = $1';
    const result = await this.pool.query(query, [turnoId]);
    return result.rows[0] || null;
  }

  async obtenerPorPreferenciaId(preferenciaId: string): Promise<Cobro | null> {
    const query = 'SELECT * FROM cobros WHERE preferencia_id = $1';
    const result = await this.pool.query(query, [preferenciaId]);
    return result.rows[0] || null;
  }

  async actualizarEstado(id: string, estado: Cobro['estado'], pagadoAt?: Date): Promise<Cobro | null> {
    const query = `
      UPDATE cobros 
      SET estado = $1,
  ...
  async actualizarMetodoPago(id: string, metodoPago: string, preferenciaId?: string): Promise<Cobro | null> {
    const query = `
      UPDATE cobros 
      SET metodo_pago = $1,
  ...
  async obtenerHistorialUsuario(usuarioId: string, limit: number = 20): Promise<Cobro[]> {
    const query = `
