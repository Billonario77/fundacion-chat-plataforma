...
export interface Cupon {
  ...
  codigo: string;
  descripcion?: string;
  tipo: 'porcentaje' | 'fijo' | 'gratis' | 'bolsa_horas';
  valor: number;
  entidad_id?: string;
...
export class CuponModel {
  ...
  async crear(datos: Omit<Cupon, 'id' | 'usos_actuales' | 'created_at' | 'updated_at'>): Promise<Cupon> {
    const query = `
      INSERT INTO cupones (
  ...
  async obtenerPorCodigo(codigo: string): Promise<Cupon | null> {
    const query = `
      SELECT * FROM cupones 
      WHERE codigo = $1 
      AND activo = true 
  ...
  async validarParaUsuario(codigo: string, usuarioId: string): Promise<Cupon | null> {
    const query = `
  ...
  async consumirUso(id: string): Promise<Cupon> {
    const query = `
      UPDATE cupones 
