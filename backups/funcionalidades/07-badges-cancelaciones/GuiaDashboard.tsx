...
const GuiaDashboard: React.FC = () => {
  ...
  // ============================================
  // NUEVO: Estado para saber si el guía ya vio las cancelaciones
  // ============================================
  const [cancelacionesVistas, setCancelacionesVistas] = useState(false);

  // ============================================
// Turnos cancelados por el GUÍA (calculado después de turnos)
// ============================================
const turnosCanceladosPorMi = turnos.filter(t => 
  ...
  useEffect(() => {
    ...
    socket.on('estado-turno-actualizado', (data) => {
      ...
      // Si es una cancelación, resetear badge
      if (data.estado === 'cancelado') {
        setCancelacionesVistas(false);
      }
    });
  ...
  // ============================================
  // NUEVO: Marcar cancelaciones como vistas al entrar a la pestaña
  // ============================================
  useEffect(() => {
    if (pestañaActiva === 'cancelados' && turnosCanceladosPorMi.length > 0) {
      setCancelacionesVistas(true);
    }
  ...
  // ============================================
  // NUEVO: Determinar si hay cancelaciones nuevas (no vistas)
  // ============================================
  const hayCancelacionesNuevas = !cancelacionesVistas && turnosCanceladosPorMi.length > 0;
