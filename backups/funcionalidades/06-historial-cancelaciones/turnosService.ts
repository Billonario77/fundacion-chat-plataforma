...
export const usuarioService = {
  ...
  // Solicitar nuevo apoyo
    solicitarApoyo: async (tipo: string, mensaje: string, fechaPreferida?: string) => {
    const token = localStorage.getItem('token');
    const response = await axios.post(
      `${API_URL}/turnos/solicitar`,
      { tipo, mensaje, fechaPreferida }, // <-- AGREGAR fechaPreferida
      { headers: { Authorization: `Bearer ${token}` } }
    );
    return response.data;
  },

  // Reprogramar un turno cancelado (accesible desde usuarioService también)
  reprogramarTurno: async (
    turnoId: string, 
    preferencia?: string, 
    fecha_preferida?: string, 
